...
<?php
	print("<BR>");
	$sql="SELECT COUNT(cod_cat) FROM categoria";
	$num=mysql_query($sql) or die(mysql_error());
	$row=mysql_fetch_array($num);
	$numcategorie=$row['COUNT(cod_cat)'];
	$contatore_elementi_stampati=1;
	$k=0;
	$contatorevero=(int)($numcategorie/3);
	if(($numcategorie%3)>0) $k++;
	$contatorevero=$contatorevero+$k;
	print("<table border=\"0\">");
	for($i=1; $i<=$contatorevero; $i++) { 
	print("<tr>");
		for($j=0; $j<3; $j++) {
		if($contatore_elementi_stampati<=$numcategorie) { 
			$sql="SELECT descrizione FROM categoria WHERE cod_cat=\"".$contatore_elementi_stampati."\"";
			$query=mysql_query($sql) or die(mysql_error());
			$row=mysql_fetch_array($query);
			$descrizione=$row['descrizione'];
			print("<td>");
			print("<a href=\"categoria.php?cod_cat=".$contatore_elementi_stampati."\"><img src=\"categoria/".$contatore_elementi_stampati.".jpg\" width=160 height=160 /></a>");
			print("<h1>".$descrizione."</h1>");
			print("</td>");
