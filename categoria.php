...
<div class="post">
		<?php
		if(isset($_GET['cod_cat'])==0) header("Location: index.php"); 
		if(is_numeric($_GET['cod_cat'])==0) header("Location: index.php"); 
		$sql="SELECT COUNT(DISTINCT(subcategoria)) FROM articolo WHERE categoria=\"".$_GET['cod_cat']."\"";
		$result=mysql_query($sql) or die(mysql_error());
		$raw=mysql_fetch_array($result);
		$numsottocategorie=$raw['COUNT(DISTINCT(subcategoria))'];
		$sql="SELECT DISTINCT(subcategoria) FROM articolo WHERE categoria=\"".$_GET['cod_cat']."\"";
		$result=mysql_query($sql) or die(mysql_error());
		$contatore_elementi_stampati=1;
		$k=0;
		$contatorevero=(int)($numsottocategorie/3);
		if(($numsottocategorie%3)>0) $k++;
		$contatorevero=$contatorevero+$k;
		print("<table border=\"0\">");
		for($i=1; $i<=$contatorevero; $i++) { 
		print("<tr>");
			for($j=0; $j<3; $j++) {
			if($contatore_elementi_stampati<=$numsottocategorie) { 
					$row = mysql_fetch_object($result);
					$contatore_elementi_stampati++;
					print("<td>");
					print("<a href=\"prodotti.php?cod_subcat=".$row->subcategoria."\"><img src=\"sottocategoria/".$row->subcategoria.".jpg\" width=150 height=150 /></a>");
					
     $subcat="SELECT sottocategoria FROM subcategoria_cod_sub WHERE cod_sub=\"".$row->subcategoria."\"";
     $querysubcat=mysql_query($subcat) or die(mysql_error());
     $querysubcat=mysql_fetch_array($querysubcat);
     $subcategoria=$querysubcat['sottocategoria'];
     print("<p>".$subcategoria."</p>");
